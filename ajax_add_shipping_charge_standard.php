...
try {
	...
	if ($s != "" && $zip != "" && $country != "") {
		...
		if ($TEMPVAL) {
			...
			$result = $ups->getRate($from_zip, $destination_zip, $dest_country_code, $services, $length, $width, $height, $total_weight, $stateCode);
	...
	/* 	$response['shipping_charge'] = $shipping_price;
	$response['shipping'] = ' ('.$Grand_total_weight.'lbs)'.CUR.$INCLUDE_SHIPPING_PRICE_CAL.' '; */
	...
	$response = array(
		...
		'shipping' => ' (' . $Grand_total_weight . 'lbs)' . CUR . $INCLUDE_SHIPPING_PRICE_CAL . ' ',
		'from_zip' => $from_zip,
