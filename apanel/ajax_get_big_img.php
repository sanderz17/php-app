<?php
	...
	$ctable_where .= "isDelete=0 AND product_id=".$prod_id;
...
<form action="" name="frm" id="frm" method="post">
    ...
    <input type="hidden" name="hdndb" id="hdndb" value="<?php echo $ctable; ?>">
		<?php
			$db->getDeleteButton();
			$db->getAddButton($page,$ctable ,ADMINURL."add-big-img/add/".$prod_id.'/');
		?>
			...
			<tbody>
				...
				if(@count($ctable_r)>0){
					...
					<tr>
						...
						<td>
							<?php //echo $ctable_d['title'];
								if($ctable_d['image_path']!="" )
								{
									?>
									<img src="<?php echo SITEURL.ACCESSORIE.$ctable_d['image_path']; ?>" width="200" >
									<?php
								}
							?></td>
						<td>
	                		<a href="<?php echo ADMINURL; ?>add-<?php echo $page; ?>/edit/<?php echo $ctable_d['id']; ?>/<?php echo $prod_id; ?>/" title="Edit" class="btn-gradient-dark p-1"><i class="mdi mdi-lead-pencil"></i></a>
                    		<a onClick="del_conf('<?php echo $ctable_d['id']; ?>',<?php echo $prod_id; ?>);" title="Delete" class="btn-gradient-danger p-1" style="color: #fff;"><i class="mdi mdi-delete"></i></a>
    					</td>
					</tr>
					<?php
					}
		...
		<?php 
			$db->getDeleteButton();
			$db->getAddButton($page, $ctable1,ADMINURL."add-big-img/add/".$prod_id.'/');
