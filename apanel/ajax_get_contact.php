<?php
	...
	if(isset($_REQUEST['searchName']) && $_REQUEST['searchName']!=""){
		$ctable_where .= " (
			...
			query_type like '%".$_REQUEST['searchName']."%'
		) AND ";
	}
			...
			<tbody>
				...
				if(@mysqli_num_rows($ctable_r)>0){
					...
					<tr>
						...
						<td><?php echo $ctable_d['firstname']." ".$ctable_d['lastname']; ?></td>
						<td><?php echo $ctable_d['email']; ?></td>
						<td><?php echo $ctable_d['number']; ?></td>
						<td><?php if($ctable_d['isAttended'] == 0){ echo "<div align='center'><a title='Not Attended' style='pointer-events: none;'><i class='mdi mdi-close'></i></a></div>"; }else{ echo "<div align='center'><a title='Attended' style='pointer-events: none;'><i class='mdi mdi-check'></i></a></div>"; } ?></td>
						<td>
